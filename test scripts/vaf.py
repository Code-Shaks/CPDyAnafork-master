...
def plot_vaf(t, vaf, vint, element, prefix):
    """
    Plot VAF and integrated VAF vs time.
    """
    fig, ax1 = plt.subplots(figsize=(6, 4))
    ax1.plot(t, vaf, '-', color='C0', label='VAF')
    ax1.set_xlabel('Time (ps)')
    ax1.set_ylabel('VAF (Å²/ps²)', color='C0')
    ax1.tick_params(axis='y', labelcolor='C0')

    ax2 = ax1.twinx()
    ax2.plot(t, vint, '--', color='C1', label='∫VAF')
    ax2.set_ylabel('Integrated VAF (Å²/ps)', color='C1')
...
if __name__ == '__main__':
    ...
    p.add_argument('--element',   required=True,
                   help="Atom symbol for VAF (e.g. Li)")
    ...
    p.add_argument('--t_end_fit_ps', type=float, default=10,
    ...
    t, vaf, vint = compute_vaf(frames, dt, args.element, args.blocks, args.out_prefix, args.in_file)
    plot_vaf(t, vaf, vint, args.element, args.out_prefix)