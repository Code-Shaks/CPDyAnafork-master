...
def segmenter_func_lammps(first_term, last_term, pos_full, dt_value, n_timesteps, cell_full=None):
    ...
    Returns:
    - tuple: (pos, step_counts, dt, time, cell) - Segmented data
    """
    ...
    # For LAMMPS, other arrays like ke_elec, etc. are typically not available
    # Return None for these or initialize as needed
...
def data_evaluator_lammps(diffusivity_direction_choices, target_elements, pos, total_ion_array, steps):
    ...
    for direction_idx, direction in enumerate(diffusivity_direction_choices):
        ...
        # Optimize: copy only the required dimensions using the mask
        for dim in range(3):
            if dim_mask[dim]:
