...
if __name__ == '__main__':
    ...
    p.add_argument('--bomd-in', help="BOMD input file (.in) for atom order")
    ...
    if args.bomd_trj:
        if not os.path.exists(args.bomd_trj):
            raise RuntimeError(f"BOMD .mdtrj file not found: {args.bomd_trj}")
        # Use BOMD elements if provided, else fallback to None
        bomd_elements = args.bomd_elements if args.bomd_elements else None
        in_file = args.bomd_in if hasattr(args, "bomd_in") and args.bomd_in else None
